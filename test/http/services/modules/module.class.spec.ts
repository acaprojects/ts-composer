...
describe('EngineModule', () => {
    ...
    it('should expose settings', () => {
        expect(module.settings).toEqual({ star: 'death' })
    })

    it('should allow updating module settings', () => {
        const new_settings = { new: 'hope' }
        module.settings = new_settings
        expect(module.settings).not.toBe(new_settings)
        expect(module.changes.settings).toBe(new_settings)
    })
